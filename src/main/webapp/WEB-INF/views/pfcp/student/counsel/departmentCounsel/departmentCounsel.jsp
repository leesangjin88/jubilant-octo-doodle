...
<div class="sectionHeaderLine">
 	<!-- 왼쪽 제목 및 설명 -->
	<div>
		<div class="sectionHeaderTitle">학과 상담 신청 목록</div>
		<div class="sectionHeaderDescription">전체 ${count}개의 게시글</div>
	</div>

	<!-- 오른쪽 등록 버튼 -->
	<button type="button" class="submitButton" onclick="location.href='/student/counsel/departmentCounsel/departmentCounselInsert.do'">+ 등록</button>
</div>
<!-- ======================================== -->
			...
			<c:choose>
				<c:when test="${not empty counselList }">
					<c:forEach items="${counselList }" var="counselList" varStatus="status">
						<c:url value="/student/counsel/departmentCounsel/departmentCounselDetail.do" var="detailURL">
						...
						<tr class="tableRowHover" data-bs-toggle="modal" data-bs-target="${counselList.counselReqno}">
							...
							<td class="tableTd">${counselList.status }</td>
